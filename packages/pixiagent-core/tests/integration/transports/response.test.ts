/**
 * Integration tests for ResponseTransport (no dialect).
 *
...
describe('ResponseTransport conversion', () => {
  const transport = new ResponseTransport(BASE_URL, API_KEY);
...
describe.skipIf(!API_KEY)('ResponseTransport - OpenRouter', () => {
  const transport = new ResponseTransport(BASE_URL, API_KEY);
